...
# Modification par TitouaCharrier
# A placer dans le switch case

generate_qls() {
    local language=$1
    shift # Supprime le premier argument pour ne garder que les tags dans $@
    ...
    ui_widget_select -c -m -s -i "${tags_available[@]}"

    if (( ${#UI_WIDGET_RC[@]} != 0 )); then
        echo "Generating custom qls..."
        
        # Utilisation d'un bloc cat pour une structure YAML propre
        cat <<EOF > "$output_file"
- description: "Requètes relatives au langage"
- qlpack: titouancharrier/cql-green-queries-${language}
- include:
    tags:
EOF

        # Itération sur les indices retournés par le widget
        for index in "${UI_WIDGET_RC[@]}"; do
            # Récupération dynamique du tag correspondant à l'index
            local selected_tag="${tags_available[$index]}"
            echo "      - $selected_tag" >> "$output_file"
        done
...
echo -e "\e[4m [GSF GREEN QUERIES] -- Choose your language :\e[24m"
options=("Java" "Python" "Javascript" "Cpp" "C#" "Actions")
ui_widget_select -l "${options[@]}"
#echo "Return code: $?"
#echo "Selected item #${UI_WIDGET_RC}: ${options[${UI_WIDGET_RC}]}";
case ${options[${UI_WIDGET_RC}]} in
  "Java")
    echo "Downloading now Java CodeQL queries pack"
    codeql pack download titouancharrier/cql-green-queries-java

    generate_qls "java" "lang" "android"
    ;;
  "Python")
    echo "Downloading now Python CodeQL queries pack"
    codeql pack download titouancharrier/cql-green-queries-python 
    
    generate_qls "python" "lang" "ai"
    ;;
  "Javascript")
    echo "Downloading now Javascript CodeQL queries pack"
    codeql pack download titouancharrier/cql-green-queries-javascript 

    generate_qls "javascript" "lang"
    ;;
  "Cpp")
    echo "Downloading now Cpp CodeQL queries pack"
    codeql pack download titouancharrier/cql-green-queries-Cpp 

    generate_qls "Cpp" "lang"
    ;;
  "C#")
    echo "Downloading now C# CodeQL queries pack"
    codeql pack download titouancharrier/cql-green-queries-Csharp 
    ;;
  "Actions")
    echo "Downloading now Actions CodeQL queries pack"
    codeql pack download titouancharrier/cql-green-queries-Actions 
    ;;
  *)
    # Bloc par défaut (si aucun motif ne correspond)
    ;;
esac
...
if [[ "${UI_WIDGET_RC}" == "yes" ]]; then
    rm -r gsf-green-queries-db
    rm custom-queries-suite.qls
fi
